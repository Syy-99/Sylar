...
#define XX(type, len, write_fun, read_fun, base_len) { \
    ...
    ba->setPosition(0); \
    SYLAR_LOG_INFO(g_logger) << ba->toString()<<" --- "<<ba->toHexString(); \
    ...
    // XX(uint64_t, 10, writeUint64, readUint64, 1);       // 只需要50字节了 压缩了接近
#undef XX
}
...
#define XX(type, len, write_fun, read_fun, base_len) { \
    ...
    ba2->setPosition(0); \
    SYLAR_LOG_INFO(g_logger) << ba2->toString()<<" --- "<<ba2->toHexString(); \
